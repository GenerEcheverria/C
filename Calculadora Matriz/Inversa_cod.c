...
for(i=0;i<N;i++){ /*generar la matriz identidad*/
for(j=0;j<N;j++){
if(i==j) identidad[i][j]=1; /*si coinciden i y j se pone a 1*/
...
j=1;
while (matriz[0][0]==0){ /*escalonamos la matriz (ponemos las filas que empiezan por 0 abajo*/
...
control=0;
j=0;
while(control==0) { /*recorre columnas*/
if(matriz[i][j]!=0) { /*busca pivote (elemento distinto de 0)*/
if(matriz[i][j]!=1) {
